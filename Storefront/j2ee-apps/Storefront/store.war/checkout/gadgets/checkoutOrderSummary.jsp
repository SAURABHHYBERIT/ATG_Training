    ...
    <c:if test="${not empty commerceItems || not empty submittedOrder}">
      <ul>
       <li class="atg_store_orderSubTotals">
        ...
        <dsp:getvalueof var="paymentSet" param="paymentSet"/>
        <c:choose>
          <c:when test="${empty paymentSet}">
          	<c:set var="paymentSet" value="false" />
          </c:when>
        </c:choose>
        
		<c:choose>
			<c:when test="${(creditAvailable) and (order.creditEligible) and (empty submittedOrder)}">
				<dl class="shipping">
					<dt>Credit payment available!</dt>
				</dl>
                
				<c:choose>
					<c:when test="${paymentSet}">
					...
					<c:otherwise>
						<dl class="shipping">
							<dt>Use credit?</dt>
							<dd>
								<dsp:input  iclass="checkbox" bean="ShoppingCart.current.useCredit" type="checkbox"
									name="atg_store_useCredit" id="atg_store_useCredit" />
			...
			<c:otherwise>
				<dl class="shipping">
					<dt>Not eligible for credit</dt>
					<dsp:input bean="ShoppingCart.current.useCredit" type="hidden" value="false" />
				</dl>
			</c:otherwise>
		</c:choose>
